...
bool Uebertragung::ladeBinFile() {
    ...
    if ((pfad.length() > 3) && (pfad.at(pfad.length()-4) == '.') && (pfad.at(pfad.length()-3) == 'b') && (pfad.at(pfad.length()-2) == 'i') && (pfad.at(pfad.length()-1) == 'n')) {
        dateipfad = pfad;
    } else {return false;}

  
  
  std::streampos anzBytes;
  char * byteListe;

  std::ifstream file (dateipfad, std::ios::in|std::ios::binary|std::ios::ate); //Setzen des Filepointers an letzte Stelle um Größe zu ermitteln
  
  if (file.is_open())
  {
    anzBytes = file.tellg();
    byteListe = new char [anzBytes]; //RAM Block in Größe der Datei allokieren
    file.seekg (0, std::ios::beg); //File Pointer auf erste Stelle setzen
    file.read (byteListe, anzBytes);
    file.close();

    bloecke = new Block[(anzBytes/blockgroesse)+1];
    int verbleibendeBytes = anzBytes;
    anzBloecke = (anzBytes/blockgroesse)+1; 

    for (int i=0; i<(anzBytes/blockgroesse)+1; ++i) {
        bloecke[i].setBlocknummer(i);
        if (verbleibendeBytes>blockgroesse) {
            bloecke[i].setBlocklaenge(blockgroesse);
        } else bloecke[i].setBlocklaenge(verbleibendeBytes);
        verbleibendeBytes -= blockgroesse;

        char* zugeschnitten = new char[blockgroesse];
        std::copy(byteListe+i*blockgroesse, byteListe+i*blockgroesse+bloecke[i].getBlocklaenge(), zugeschnitten);
        bloecke[i].setDaten(zugeschnitten);
        bloecke[i].berechneParitaet();
    }
    delete[] byteListe;

    std::cout << "Datei mit " << anzBloecke<< " Blöcken erfolgreich geladen" << std::endl;
    //debugBlockInhalt(bloecke,anzBytes);
...
void Uebertragung::schreibeBinFile() {
    /*
    std::cout << "wird abgeschlossen...";
    // Ausklammern start
    for (unsigned long int i=0; i<getAnzBloecke(); ++i) {
        std::cout << "Block " << i << std::endl; 
        for (unsigned long int j=0; j<bloecke[i].getBlocklaenge(); ++j) {
            std::cout << std::bitset<8>(bloecke[i].getDaten()[j]) << std::endl;
        }
    }
    // ausklammern ende
    std::ofstream file("EmpfangeneDatei.bin",  std::ios::binary | std::ios::ate);
    if (file.is_open()) {
        for (unsigned long int i=0; i<getAnzBloecke(); ++i) {
            //std::cout << std::bitset<8>(bloecke[i].getDaten()[0]) << std::endl;
            //std::cout << std::bitset<8>(bloecke[i].getDaten()[16]) << std::endl;
            file.write(bloecke[i].getDaten(), bloecke[i].getBlocklaenge()); 
        }
    file.close();
    } else {
    ...
    if (!file.good()) {
        std::cout << "Es ist ein Fehler beim Schreiben der Datei aufgetreten" << std::endl;
    } else {
...
void Block::setDaten(char* d) {
    ...
    /*
    std::cout << "Block " << blocknummer << std::endl;
    for (unsigned long int i=0; i<blocklaenge; ++i) {
        std::cout << i << ": " << std::bitset<8>(daten[i]) << std::endl;
    }
    */
    //std::cout << "Setze " << std::bitset<8>(d[0]) << std::endl;
    //std::cout << "Setze " << std::bitset<8>(d[16]) << std::endl;
    berechneParitaet();
}

char* Block::getDaten() {
    //for (unsigned long i=0; i<blocklaenge; ++i) {
    //    std::cout << "Debug(" << i << ") " << std::bitset<8>((char) daten[i]) << std::endl;
    //}
    //std::cout << "Lese " << std::bitset<8>(daten[0]) << std::endl;
    //std::cout << "Lese " << std::bitset<8>(daten[16]) << std::endl;
